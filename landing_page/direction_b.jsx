// Direction B — Editorial / Asymmetric
// Wider canvas. Split hero. Numbered ledger. Document-like section starters.

function DirectionB({ lang }) {
  ...
  return (
    <div>
      <Nav lang={lang} onCTA={goCTA} pulse={pulse} variant="B" />
      ...
      <section className="container-wide" style={{ paddingTop: 80, paddingBottom: 80 }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.15fr) minmax(0, 1fr)", gap: 64, alignItems: "center" }}>
          <div>
            ...
            <h1 style={{ fontSize: "clamp(2.8rem, 5.5vw, 5rem)", lineHeight: 1.0, letterSpacing: "-0.04em", fontWeight: 700, margin: "0 0 28px", color: "var(--ink)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 0.7fr) minmax(0, 1.3fr)", gap: 48 }}>
          ...
          <div>
            {t.steps.map(({ n, t: title, d }, i) => (
              <div key={n} style={{ display: "grid", gridTemplateColumns: "60px 1fr", gap: 24, padding: "28px 0", borderTop: "1px solid var(--line)", transition: `opacity 0.6s ease ${i * 140}ms, transform 0.6s ease ${i * 140}ms`, opacity: stepsIn ? 1 : 0, transform: stepsIn ? "translateY(0)" : "translateY(14px)" }}>
      ...
      {/* Waitlist */}
      <section ref={waitlistRef} className="container-wide" style={{ padding: "96px 32px" }}>
